...
%% nihbiosketch.cls requires tabu, which is unmaintained and breaks against
%% the LaTeX2e 2026-06-01 kernel's table-tagging sockets: \end{datetbl}
%% raises "Extra }, or forgotten \endgroup" and the education table is lost,
%% costing a page. ubuntu-latest ships that kernel; TeX Live on macOS may
%% still be older, so the failure is invisible locally. Roll the kernel back
%% to the last release tabu is known good against. Remove this once the class
%% moves off tabu (tabularray) or tabu itself is fixed.
\RequirePackage[2025-11-01]{latexrelease}

%% nihbiosketch.cls calls \setmainfont{Arial} during class loading, which
%% errors on systems without Arial (e.g. Linux CI). Note that {quiet} does
%% NOT rescue that: it silences fontspec's warnings, not the font-not-found
%% error. CI neutralises the class's font line instead (see build-cv.yml);
%% {quiet} is kept only to mute the ordinary shape warnings that follow.
%% Either way, the preamble below is what actually sets the font.
\PassOptionsToPackage{quiet}{fontspec}
\documentclass{nihbiosketch}

%% Use Arial when available (Mac), fall back to TeX Gyre Heros on CI
%% (metrically compatible, OpenType, ships with TeX Live via tex-gyre).
%% The fallback loads by FILENAME, not by name: XeTeX's by-name lookup
%% uses fontconfig, which doesn't index TeX Live's bundled fonts, whereas
%% filename loading resolves via kpathsea/texmf and always works on CI.
...
%% Only these three fields exist: nihbiosketch.cls defines \name,
%% \eracommons and \position, and \piinfo renders exactly those three rows.
%% There is no \orcid — the NIH biosketch form has no ORCID field, and
%% calling it raised "Undefined control sequence" on every build, macOS
%% included. ORCID is carried on the CV instead.
\name{Ehrlinger, John}
\eracommons{EHRLINJ}
\position{Associate Staff, Lead Data Scientist}
...
open-source software tools (ggRandomForests in R; hazard in SAS/C) that make these methods
...
Thoracic and Cardiovascular Surgery, where I lead a team of data engineers
and data scientists focused on cardiovascular outcomes research. In this role I
...
2015          & Clinical Assistant Professor (Joint Appointment), Cleveland Clinic
...
2023--2024    & Senior Data Scientist -- Technical Lead, Altamira Technologies,
                McLean, VA \\
2024--2026    & Assistant Staff, Lead Data Scientist, Dept.\ of Thoracic and Cardiovascular Surgery,
                Heart, Vascular \& Thoracic Institute, Cleveland Clinic, Cleveland, OH \\
2025--present & Assistant Professor of Surgery (Joint Appointment), Cleveland Clinic
                Lerner College of Medicine -- Case Western Reserve University,
                Cleveland, OH \\
2026--present & Associate Staff, Lead Data Scientist, Dept.\ of Thoracic and Cardiovascular Surgery,
                Heart, Vascular \& Thoracic Institute, Cleveland Clinic, Cleveland, OH \\
\end{datetbl}
...
ggRandomForests R package, and maintain the hazard SAS/C package, an
open-source implementation of multi-phase hazard analysis for time-to-event
decomposition originally developed at Cleveland Clinic. I additionally develop
and maintain the HVTI R package family — eleven coordinated R packages,
installed and version-checked as a unit through the hvtiR installer —
standardizing graphics, manuscript tables, analytic dataset construction,
propensity score methods, matched reference survival, and bootstrap model
building across the HVTI CORR research pipeline.
